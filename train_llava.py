...
from models.alignment_pretrained.model_with_bce_images_llava import MMModerator
...
def main(args):
    ...
    model = DDP(model, device_ids=[local_rank], output_device=local_rank,find_unused_parameters=True)
        ...
        # model.load_state_dict(torch.load(os.path.join(args.load_from, "model_state.pt")))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for loader_info in [{"loader":train_loader,"name":"VideoAudio"}]:#{"loader": faceswap_train_loader, "name": "faceswap"},,]:,{"loader":faceswap_video_train_loader,"name":"faceswapVideo"}
            ...
            for i, batch in enumerate(tqdm(data_loader, desc=f"Epoch {epoch + 1}/{args.epochs} {loader_name}")):
                ...
                # set your desired drop probability
                ...
                # Optional: clip AFTER unscale, but only once
                ...
                if writer is not None:
                    ...
                    if i < 4:
                        ...
                        # output_file = f"{log_dir}/{loader_name}_prompts.csv"
                        ...
                        for index, v in enumerate(image_recon[len(image_recon)//2:]):
                            # img = v.sigmoid().clamp(0, 1)   # recon logits → probability
                            # img = v.clamp(0, 1)   # recon logits → probability
                            # img = v.clamp(0, 1)   # mask, already [0,1]
                            # # img = v.sigmoid().clamp(0,1)
                            # to_pil(img).save(f"{log_dir}/{loader_name}_{index}_pix_diff_recon.png")
