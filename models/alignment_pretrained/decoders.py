...
class VisualDecoder32x32(nn.Module):
    def __init__(self, img_size=384, patch_size=16, n_frames=25, embed_dim=384, depth=8,
        ...
        #     384 // self.n_patch_h,                  # height grid — or video.shape[-2] if dynamic
        #     384 // self.n_patch_w                    # width grid — or video.shape[-1] if dynamic
        # )
...
class VisualDecoder8x8(nn.Module):
    def __init__(self, img_size=384, patch_size=16, n_frames=25, embed_dim=384, depth=8,
        num_heads=6, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        norm_layer="LayerNorm", init_values=1., tubelet_size=5, encoder_embed_dim=768
    ):
...
class VisualDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.patch_size   = patch_size
        self.tubelet_size = tubelet_size
        self.n_patch_h    = img_size // patch_size       # e.g. 384//16 = 14
...
class MaskEncoder(nn.Module):
 ...
 # nn.Conv3d(
        ...
        self.video_grid = (
            ...
            384 // video_patch_height,                  # height grid — or video.shape[-2] if dynamic
            384 // video_patch_width                    # width grid — or video.shape[-1] if dynamic
        )
        pos_3d = get_3d_sincos_pos_embed(
            embed_dim = dim,
            grid_size = self.video_grid,
