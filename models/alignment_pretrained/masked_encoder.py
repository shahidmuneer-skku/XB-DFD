...
class WaveGLU(nn.Module):
    def __init__(self):
        super(WaveGLU, self).__init__() 
        self.w1 = nn.Parameter(torch.ones(1), requires_grad=True)
        self.w2 = nn.Parameter(torch.ones(1), requires_grad=True)

    def forward(self, x):
        x_lin, gate = x.chunk(2, dim=-1)      # both [*, inner_dim]
        return x_lin * (self.w1 * torch.sin(gate) + self.w2 * torch.cos(gate))  # WaveAct gate
...
def FeedForward(dim, mult = 4):
    ...
    return nn.Sequential(
        LayerNorm(dim),
        # RMS_norm(dim, channel_first=False, images=False, bias=False),
        nn.Linear(dim, inner_dim * 2, bias = False),
        GEGLU(),
        #  WaveGLU(),
        # WaveAct(),
        # LayerNorm(inner_dim),
        # RMS_norm(inner_dim , channel_first=False, images=False, bias=False),
...
class Attention(nn.Module):
    def __init__(
        ...
        # self.norm = RMS_norm(dim, channel_first=False, images=False, bias=False)
...
#     def forward(self, x):
#         b, h, w, d = x.shape   # (batch, h_patches, w_patches, patch_dim)
#         n = h * w
#         idx = torch.randperm(n, device=x.device)  # random permutation
#         x = x.reshape(b, n, d)[:, idx, :]         # shuffle patches
#         return x, idx   # optionally return idx if you want to undo later

class RandomPatchShuffle(nn.Module):
    def forward(self, x):
        """
        x: [B, H, W, D] or [B, T, H, W, D]
        """
        b, *spatial_dims, d = x.shape
        n = int(np.prod(spatial_dims))  # total patches per sample

        # Generate random permutation
        idx = torch.randperm(n, device=x.device)

        # Flatten, shuffle, then restore grid
        x = x.reshape(b, n, d)[:, idx, :]
        x = x.reshape(b, *spatial_dims, d)
        return x
...
class MaskEncoder(nn.Module):
 ...
 # nn.Conv3d(
        ...
        self.image_to_tokens = nn.Sequential(
            ...
            # RandomPatchShuffle() if self.training else nn.Identity(),
        ...
        device = audio.device if audio is not None else (
            video.device if video is not None else (
                image.device if image is not None else (
                    text_tokens.device if text_tokens is not None else "cuda:0"
                )
            )
        )
        if audio is not None:
