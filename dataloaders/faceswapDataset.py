...
def load_faceforensics_dataset(root: Path,
                    # fake_root: Path,
                    label,
                    # quality_tag: str,
                    file_list: list):
...
def read_video(path: str):
    ...
    audio_path = "/media/NAS/USER/cropped_video.wav"
    try:
...
class FaceswapDataset(Dataset):
    ...
    def __init__(self, base_dir, partition="train",take_datasets="1,2,3,4,5,6", max_len=64000, frame_rate=1, n_mfcc=40):
        ...
        # self.image_aug = self.image_aug = K.AugmentationSequential(
        #         K.Resize((224, 224)),
        ...
        #         K.RandomJPEG(jpeg_quality=(30.0, 70.0), p=0.8),
        #         K.RandomElasticTransform(alpha=(0.8, 1.2), sigma=(8.0, 12.0), p=0.7),
        #         data_keys=["input"],
        #         same_on_batch=False,
            # )
        ...
        if partition=="test":
            df = pd.read_csv("/media/NAS/DATASET/DFWild-Cup/valid.csv")
        ...
        if partition=="train":
            ...
            print(f"Total files in after neural texture {partition} are {len(self.file_list)}")
            
            # # # List and sort filenames
            # original_files = sorted(os.listdir(original_root))
            # faceswap_files = sorted(os.listdir(faceswap_root))

            # # # Make sure the lengths match
            # # assert len(original_files) == len(faceswap_files), "Mismatch between real and fake file counts!"

            # # Merge corresponding real and fake paths
            # # paired_paths = []
            # for orig_file, fake_file in zip(original_files, faceswap_files):
            #     orig_path = os.path.join(original_root, orig_file)
            #     fake_path = os.path.join(faceswap_root, fake_file)
            #     if os.path.exists(orig_path) and os.path.exists(fake_path):
            #         self.file_list.append({
            #                 "path":orig_path,
            #                 "low_quality_file":low_quality_dataset[orig_file.split(".")[0]],
            #                 "label":0
            #         })
            #         self.file_list.append({
            #                 "path":fake_path,
            #                 "low_quality_file":low_quality_dataset[fake_file.split(".")[0]],
            #                 "label":1
            #         })

            # root = "/media/NAS/DATASET/celeba_hq/train/male"
            ...
            # root = "/media/NAS/DATASET/celeba_hq/train/female"
            # for path in os.listdir(root):
            #     path = f"{root}/{path}"
            #     if os.path.exists(path):
            #         self.file_list.append({
            ...
            # root = "/media/NAS/DATASET/celeba/celeba/img_align_celeba/"
            # i = 0
            # for path in os.listdir(root):
            #     path = f"{root}/{path}"
            #     i+=1
    ...
    def __getitem__(self, index):            
        try:
            ...
            file_path = data["path"]
            low_quality_file = data["low_quality_file"] if "low_quality_file" in data else None
            ...
            if low_quality_file is not None:
                image_aug = Image.open(low_quality_file)#self.image_aug(image)
                image_aug = self.image_no_aug(image_aug)
            else: 
